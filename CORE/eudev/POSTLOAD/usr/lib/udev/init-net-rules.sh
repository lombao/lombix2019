...
RULES=/usr/etc/udev/rules.d/70-persistent-net.rules 
...
for NIC in $DEVICES; do
     ...
     export MATCHADDR="$(/usr/bin/cat $NIC/address)" # Read MAC address

     VENDOR=$(echo $MATCHADDR | /usr/bin/cut -c-9)
     if ignore_if; then continue; fi

     export MATCHDEVID="$(/usr/bin/cat $NIC/dev_id)"
     export MATCHIFTYPE="$(/usr/bin/cat $NIC/type)"  # Read interface type
     comment
